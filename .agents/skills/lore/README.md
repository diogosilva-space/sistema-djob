...
lore maintains a single source of truth (`.lore/`) and projects it into whatever config files your agents already read. It tracks _why_ decisions were made, not just _what_ the code does, and keeps that history across sessions and tools.
...
The agent reads the commit messages and tells you _why_ — without you having to manually dig through `git log`.
