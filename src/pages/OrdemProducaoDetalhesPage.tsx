        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              <div
                className={`text-[10px] font-bold uppercase py-0.5 px-2 rounded-full border ${getPrioridadeColor(op.prioridade)}`}
              >
                {op.prioridade}
              </div>
            </div>
              ...
              <div className="relative space-y-8 before:absolute before:inset-0 before:ml-[1.25rem] before:-translate-x-px before:h-full before:w-0.5 before:bg-gradient-to-b before:from-muted before:via-muted before:to-transparent">
                {op.etapas.map((etapa, idx) => (
                  <div key={etapa.id} className="relative flex items-start gap-6 group">
                    <div
                      className={`flex items-center justify-center w-10 h-10 rounded-full border-4 border-background shadow-sm transition-all z-10
                      ${
                    ...
                    <div className="flex-1 pt-1">
                      <div className="flex items-center justify-between">
                        <h3
                          className={`text-sm font-bold ${etapa.status === 'Pendente' ? 'text-muted-foreground' : 'text-foreground'}`}
                        >
                          {etapa.nome}
                        </h3>
                        <Badge
                          ...
                          {etapa.dataInicio && (
                            <div className="space-y-1">
                              ...
                              <div className="flex items-center gap-1.5 text-xs">
                                ...
                                <span>
                                  {format(etapa.dataInicio, 'dd/MM/yyyy HH:mm', { locale: ptBR })}
                                </span>
                              </div>
                            </div>
                          )}
                          {etapa.dataFim && (
                            <div className="space-y-1">
                              ...
                              <div className="flex items-center gap-1.5 text-xs">
                                ...
                                <span>
                                  {format(etapa.dataFim, 'dd/MM/yyyy HH:mm', { locale: ptBR })}
                                </span>
                              </div>
                            </div>
                          )}
                      ...
                      {etapa.status === 'Pendente' &&
                        idx > 0 &&
                        op.etapas[idx - 1].status === 'Concluído' && (
                          <div className="mt-4">
                            <Button
                              size="sm"
                              variant="outline"
            ...
            <CardContent className="pt-6 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="flex flex-col">
                  ...
                  <span className="text-[13px] font-medium">
                    {format(op.dataEmissao, 'dd/MM/yyyy')}
                  </span>
                </div>
              </div>
              ...
              <div className="flex flex-col">
                ...
                <span className="text-[15px] font-bold">
                  {format(op.dataEntregaPrometida, "dd 'de' MMMM", { locale: ptBR })}
                </span>
                <span className="text-[11px] text-muted-foreground mt-0.5">
                  Ano: {format(op.dataEntregaPrometida, 'yyyy')}
                </span>
              </div>
          ...
          <Card className="shadow-sm border-muted/40">
            ...
            <CardContent className="pt-4">
              <p className="text-xs text-muted-foreground leading-relaxed">
                {op.observacoes ||
                  'Nenhuma observação interna registrada para esta ordem de produção.'}
              </p>
            </CardContent>
          </Card>
        </div>
